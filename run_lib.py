...
def train(config, workdir):
    ...
    if config.training.sde.lower() == "vpsde":
        ...
        sde = sde_lib.subVPSDE(
            beta_min=config.model.beta_min, beta_max=config.model.beta_max, N=config.model.num_scales
        )
        sampling_eps = 1e-3
...
def evaluate(config, workdir, eval_folder="eval"):
    ...
    train_ds, eval_ds, _ = datasets.get_dataset(
        config, uniform_dequantization=config.data.uniform_dequantization, evaluation=True
    )
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
            ...
            for repeat in range(bpd_num_repeats):
                ...
                for batch_id in range(len(ds_bpd)):
                    ...
                    with tf.io.gfile.GFile(
                        os.path.join(eval_dir, f"{config.eval.bpd_dataset}_ckpt_{ckpt}_bpd_{bpd_round_id}.npz"), "wb"
                    ) as fout:
                        io_buffer = io.BytesIO()
        ...
        if config.eval.enable_sampling:
            ...
            for r in range(num_sampling_rounds):
                ...
                samples = np.clip(samples.permute(0, 2, 3, 1).cpu().numpy() * 255.0, 0, 255).astype(np.uint8)
