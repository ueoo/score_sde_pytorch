...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
    ...
    if batch_size % jax.device_count() != 0:
        raise ValueError(
            f"Batch sizes ({batch_size} must be divided by" f"the number of devices ({jax.device_count()})"
        )
    ...
    if config.data.dataset in ["FFHQ", "CelebAHQ"]:
        ...
        def preprocess_fn(d):
            sample = tf.io.parse_single_example(
                d,
                features={"shape": tf.io.FixedLenFeature([3], tf.int64), "data": tf.io.FixedLenFeature([], tf.string)},
            )
